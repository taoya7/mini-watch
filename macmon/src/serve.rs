...
fn to_prometheus(m: &Metrics, soc: &SocInfo) -> String {
  ...
  gauge!(out, "macmon_fans_number", "Sum of fan speeds in RPM", m.fans_number);
  out.push_str("# HELP macmon_fan_speed_rpm Fan speed in RPM\n# TYPE macmon_fan_speed_rpm gauge\n");
  for fan in &m.fans {
    out.push_str(&format!(r#"macmon_fan_speed_rpm{{{l},fan="{}"}} {}"#, fan.id, fan.rpm));
    out.push('\n');
  }
  out.push('\n');
