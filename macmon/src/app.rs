...
use crate::metrics::{FanMetrics, Metrics, Sampler, zero_div};
...
impl App {
  ...
  fn render(&mut self, f: &mut Frame) {
    let fans = if self.fans.is_empty() {
      "".to_string()
    } else if self.fans.len() == 1 {
      format!(" fan {} RPM", self.fans[0].rpm)
    } else {
      let rpms = self.fans.iter().map(|x| x.rpm.to_string()).collect::<Vec<_>>().join("/");
      format!(" fans {rpms} RPM")
    };
    let label_l = format!(
      "{} ({}{}+{}{}+{}GPU {}GB{})",
      self.soc.chip_name,
